...
// };
const getAction = () => {
  actions = ["PTR_TITLE_OWNER_DETAILS", "PTR_TITLE_PET_DETAILS", "PTR_TITLE_ADDRESS_DETAILS", "PTR_TITLE_DOCUMENT_DETAILS", "Summary"];
};
const Timeline = ({ currentStep, flow = "" }) => {
  const { t } = useTranslation();
  const isMobile = window.Digit.Utils.browser.isMobile();
  getAction();
  useEffect(() => {
  ...
  return (
    <div className="timeline-container" style={isMobile ? {} : { maxWidth: "960px", minWidth: "640px", marginRight: "auto" }}>
      {actions.map((action, index, arr) => (
        <div className="timeline-checkpoint" key={index}>
          <div className="timeline-content">
            <span className={`circle ${index <= currentStep && "active"}`}>{index < currentStep ? <TickMark /> : index + 1}</span>
            <span className="secondary-color">{t(action)}</span>
          </div>
          {index < arr.length - 1 && <span className={`line ${index < currentStep && "active"}`}></span>}
        </div>
      ))}
    </div>
  );
